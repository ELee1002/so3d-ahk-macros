/*  爆破廢墟-終 (AHI 版)
 *  爆破山脈：15 秒連打 1 → 按 2 x3 → 每 N 輪連按空白 X 秒
 *  空白階段若偵測 Lib\滿包 → 執行丟廢墟垃圾（三頁）
...
InitAllCfg() {
    ...
    global lockNX, lockNY, gemIdx, lastStuckName, lastStuckX, lastStuckY, stuckCount, skipPageDiscard
...
TryDropOnFullBagDuringSpace() {
    global running, bagDropDoneThisSpace, trainPhase, spaceEndTick, nextTick, nextBagCheck
...
InitApp() {
    ...
    ini()
    BuildMacroPanel("爆破廢墟-終", infoText, hotkeyText, StartTrain, StopTrain)
    state()
